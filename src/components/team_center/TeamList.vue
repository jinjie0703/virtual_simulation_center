<template>
  <div>
    ...
    <!-- 空状态 -->
    <div v-else-if="paginatedTeams.length === 0" class="empty-state">
      <slot name="empty-state">
        <i class="fas fa-search"></i>
        <h3>没有找到相关团队</h3>
        <p>请尝试其他搜索条件</p>
      </slot>
      <button @click="clearFilters" class="reset-button">
    ...
    <div v-else class="team-list-container">
      <transition-group name="team-list" tag="div">
        <div v-for="team in paginatedTeams" :key="team.id" class="team-item-card">
          <slot name="team-card" :team="team"></slot>
        </div>
      </transition-group>
    </div>
  </div>
  ...
  computed: {
    filteredTeams() {
      ...
      if (this.filterOption === 'recent') {
        // 已经按创建日期排序，不需要额外操作
      } else if (this.filterOption === 'popular') {
        result.sort((a, b) => b.memberCount / b.maxMembers - a.memberCount / a.maxMembers)
      }
    ...
    filteredTeams: {
      handler(newVal) {
        ...
        this.debounceTimer = setTimeout(() => {
          this.$emit('update-total-items', newVal.length)
        }, 50)
      },
      immediate: true,
    },
